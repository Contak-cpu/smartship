...
import { StockTiendaCliente, StockTiendaClienteInput, TiendaCliente } from '../types';
import {
  obtenerStockTienda,
  guardarStockTienda,
  eliminarStockTienda,
} from '../services/stockTiendasClientesService';
import { obtenerTiendaPorId, isProPlusUser } from '../services/tiendasClientesService';
import LockedOverlay from '../components/common/LockedOverlay';

const StockTiendaPage: React.FC = () => {
  const { tiendaId } = useParams<{ tiendaId: string }>();
  const navigate = useNavigate();
  const { username, userId, userLevel } = useAuth();
  const [tienda, setTienda] = useState<TiendaCliente | null>(null);
  const [stock, setStock] = useState<StockTiendaCliente[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingStock, setEditingStock] = useState<StockTiendaCliente | null>(null);
  const [formData, setFormData] = useState<StockTiendaClienteInput>({
    sku: '',
    nombreproducto: '',
    cantidad: 0,
    equivalencia: 1,
  ...
  useEffect(() => {
    if (userId && tiendaId && canAccess) {
      cargarTienda();
      cargarStock();
    }
  }, [userId, tiendaId, canAccess]);

  const cargarTienda = async () => {
    if (!tiendaId || !userId) return;
    
    try {
      const data = await obtenerTiendaPorId(tiendaId, userId);
      if (!data) {
        showMessage('error', 'Tienda no encontrada o no tienes acceso');
        navigate('/tiendas-clientes');
        return;
      }
      setTienda(data);
    } catch (error) {
      console.error('Error cargando tienda:', error);
      showMessage('error', 'No se pudo cargar la tienda');
    }
  };
  ...
  const cargarStock = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await obtenerStockTienda(tiendaId);
      setStock(data);
    } catch (error) {
  ...
  const handleOpenModal = (item?: StockTiendaCliente) => {
    if (item) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userId || !username || !tiendaId) {
      showMessage('error', 'Usuario o tienda no identificado');
      return;
    }

    // Verificar permisos de edición si es una tienda compartida
    if (tienda && tienda.es_compartida && !tienda.puede_editar) {
      showMessage('error', 'No tienes permisos para editar el stock de esta tienda');
      return;
    }
    ...
    try {
      await guardarStockTienda(userId, username, tiendaId, formData);
      showMessage('success', editingStock ? 'Stock actualizado' : 'Producto agregado al stock');
      cargarStock();
      handleCloseModal();
    } catch (error: any) {
      console.error('Error guardando stock:', error);
      const errorMessage = error?.message || 'No se pudo guardar el stock';
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-4 sm:p-6 lg:p-8 transition-colors duration-300">
        <div className="max-w-7xl mx-auto">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white">
                    Stock - {tienda.nombre_tienda}
                    {tienda.es_compartida && (
                      <span className="ml-3 text-sm font-normal text-purple-600 dark:text-purple-400">
                        (Compartida)
                      </span>
                    )}
                  </h1>
                  <p className="text-gray-600 dark:text-gray-400 text-sm sm:text-base">
                    {tienda.es_compartida 
                      ? (tienda.puede_editar 
                          ? 'Administra el inventario de esta tienda compartida' 
                          : 'Visualiza el inventario de esta tienda compartida (solo lectura)')
                      : 'Administra el inventario de esta tienda'}
                  </p>
                </div>
              </div>
              {(!tienda.es_compartida || tienda.puede_editar) && (
                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/50 dark:to-purple-800/30 rounded-2xl p-6 border-2 border-purple-200 dark:border-purple-500/30">
              ...
              {stock.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-500 dark:text-gray-500 text-sm mb-4">
                    Comienza agregando productos al inventario de esta tienda
                  </p>
                  {(!tienda.es_compartida || tienda.puede_editar) && (
                    <button
                      onClick={() => handleOpenModal()}
                      ...
                      {stock.map((item) => (
                        ...
                        >
                          <td className="py-4 px-6">
                            <span className="text-purple-600 dark:text-purple-400 font-bold">{item.sku}</span>
                          </td>
                          ...
                          <td className="py-4 px-6">
                            {(!tienda.es_compartida || tienda.puede_editar) ? (
                              ...
                              <span className="text-gray-400 dark:text-gray-500 text-sm">Solo lectura</span>
                            )}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.equivalencia || 1}
                    onChange={(e) => setFormData({ ...formData, equivalencia: parseInt(e.target.value) || 1 })}
                    className="w-full px-4 py-3 bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:outline-none focus:border-purple-500"
                    placeholder="1"
                    min="1"
                  />
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Factor de equivalencia para el descuento (por defecto: 1)
                  </p>
                </div>
...
export default StockTiendaPage;
